...
accSettings.onclick = function (e) {
  if (
    ...
    accSettingsText.innerHTML === "Open Account Settings"
  ) {
...
function L(_) {
  if (_ === 0) {
    ...
    document.getElementById("accountPasswordInput").placeholder =
...
(function (e) {
  function K() {
    try {
      document.querySelector("#nameInput").value = localStorage.getItem("name") || "Player_0";
        ...
        document.getElementById("errorText").innerHTML = "You can't sign in without cookies.";
    ...
    fetch(`/userData?password=${localStorage.getItem("password")}?username=${localStorage.getItem("username")}`)
      ...
      .catch(function (h) {
        document.getElementById("PlayerGold").innerHTML = "Error.";
        document.getElementById("PlayerRank").innerHTML = "Error.";
      });
  ...
  function connect() {
    ws = new WebSocket("wss://jbivk4-8000.preview.csb.app/websocket");
