(() => {
  ...
  app.post("/data", (e, t) => {
    fs.readFile("." + process.env.P, "utf-8", (k, h) => {
      let u = JSON.parse(`[${h.replace(/,$/, "")}]`),
        ...
        f = Object.assign(e.body, {
          loggedIn: false,
          PlayerGold: 0,
          ip: e.headers["x-forwarded-for"].split(",").shift(),
        });
      if (k) throw k;
      if (e.body.url == "create") {
        try {
          if (!(e.body.username || e.body.password || e.body.password2)) {
            ...
            e.body.username.length > 15 ||
            e.body.username.length < 3
          ) {
            t.sendStatus(404);
          } else {
            t.sendStatus(200);
            fetch("https://jbivk4-8000.preview.csb.app" + process.env.P)
              .then((t) => t.text())
              .then((h) => {
    ...
    app.use("/userData", function (e, t) {
      ...
      fs.readFile("." + process.env.P, "utf-8", (k, h) => {
        ...
        for (i = 0; i < x.length; i++) {
          if (
            x[i].password.toLowerCase() === d.password.toLowerCase() &&
            x[i].username.toLowerCase() === d.username.toLowerCase()
          ) {
            x[i].PlayerRank = i + 1;
            t.status(200).send(x[i]);
            break;
          }
          if (i >= x.length) {
            t.status(200).send({
    ...
    app.use("/error", function (e, t) {
      t.sendFile(__dirname + "/views/error.html");
    }),
    app.use("/Privacy", function (e, t) {
      t.sendFile(__dirname + "/public/privacy.txt");
    }),
    ...
    app.use("/Terms", function (e, t) {
      t.sendFile(__dirname + "/public/terms.txt");
    }),
    app.use("/Version", function (e, t) {
      t.sendFile(__dirname + "/public/version.txt");
    }),
      ...
      if (message.length < 1000) {
        ...
        switch (msg[0]) {
          case "j":
            ...
            fetch("https://jbivk4-8000.preview.csb.app" + process.env.J)
              .then((res) => res.text())
              .then((data) => {
                const Banned = data
                  .replaceAll("\n", "")
                ...
                try {
                  ...
                  if (!isNaN(siid)) {
                    ...
                    if (!(players.admin || players.mod)) {
                      fetch(
                        "https://jbivk4-8000.preview.csb.app" + process.env.J
                      )
                        .then((res) => res.text())
                        .then((data) => {
                          var datax = data + "\n" + players.ip + ",";
          ...
          case "ih":
            var msg = msg[1][0];
            fetch(
              "https://discord.com/api/webhooks/1025959039491911721/t3VpeljGizisaObXzIHCEE5GhhPSSkhu7ohPydPLgKKJhKRhPvrphz-KHBl4PPdTLPhL",
              {
                headers: {
              ...
              .then((e) => console.log("Error reported."))
              .catch(function (e) {
